...
def get_all_laplacian_electrodes(electrode_labels):
    ...
    def get_neighbors_original_labels(label, label_to_stem, stem_to_labels):
    ...
    label_to_stem = {label: stem_electrode_name(label) for label in electrode_labels}
    ...
    laplacian_labels = [label for label, stem in label_to_stem.items() if has_neighbors(stem, stems)]
    
    # Build neighbors dict using original labels
    neighbors = {label: get_neighbors_original_labels(label, label_to_stem, stem_to_labels) for label in laplacian_labels}
...
def laplacian_rereference_batch(batch, remove_non_laplacian=True, inplace=False):
    ...
    inputs:
        batch: dictionary from SingleSessionDataset with keys:
            ...
            'metadata': dict
        remove_non_laplacian: boolean, if True, remove the non-laplacian electrodes from the data
        inplace: boolean, if True, modify the batch dictionary in place
    outputs:
        batch: dictionary with rereferenced data:
    ...
    rereferenced_data, rereferenced_labels, original_electrode_indices = laplacian_rereference_neural_data(
...
if __name__ == "__main__":
    ...
    dataset = SingleSessionDataset("vanblooijs_hermes_developmental_2023/sub-ccepAgeUMCU01/ses-1_task-SPESclin_run-021448", context_length=1.0)
    sample = dataset[0]
    ...
    print(f"\nRereferenced data shape: {rereferenced_sample['ieeg']['data'].shape}", rereferenced_sample['ieeg']['data'])
