...
class SingleSessionDataset(Dataset):
    def __init__(self, session_string, context_length): 
        ...
        self.data_file = os.path.join(os.environ["DATA_ROOT_DIR"], self.brainset, self.subject, self.session, "data.h5")
        ...
        with h5py.File(self.data_file, "r") as f:
            ...
            if "[" in session_string:
                ...
                self.time_from, self.time_to = float(self.time_from), float(self.time_to)
            else:
...
class MultiSessionDataset(ConcatDataset):
    def __init__(self, session_strings, context_length):
        """
    ...
    def _discover_dirs(self, path, require_data_h5=False):
        ...
        if not path.exists():
            return []
        dirs = [d.name for d in path.iterdir() 
    ...
    def _expand_session_pattern(self, data_root, brainset_pattern, subject_pattern, session_pattern):
        ...
        return expanded
    def _expand_session_wildcards(self, session_strings):
        """
...
class SessionBatchSampler(torch.utils.data.Sampler):
    ...
    def __iter__(self):
        ...
        for size in self.dataset_sizes:
            ...
            session_batches = [session_indices[i:i + self.batch_size] 
...
def ieeg_collate_fn(batch, cfg=None):
    """
...
class iEEGDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        with open(self.cfg.training.train_subject_trials_file, "r") as f:
            train_subject_trials = yaml.safe_load(f)
        
        print(f"Loading {len(train_subject_trials)} training sessions...")
        ...
        print(f"Total samples across {len(self.session_sizes)} sessions: {len(self.full_dataset)}")
        print(f"Session sizes: {self.session_sizes}")
